...
- `Bun.spawnSync` for CLI availability checks throws if the binary is not on PATH but returns normally on non-zero exit. Wrap in try/catch when probing `claude` or `codex` CLI availability.
